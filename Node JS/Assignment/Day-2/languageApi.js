...
//delete a single language by it's id
app.delete("/deletelang/:id", (req, res) => {
    let id = req.params.id;

    languageModel.deleteOne({_id:id}, (err, data) => {
        if(err === null)
        {
...
//update a single language by it's id
app.put("/updatelang/:id", (req, res) => {
    let id = req.params.id;
    let lang = req.body;

    languageModel.updateOne({_id:id}, lang, (err, data) => {
        if(err === null)
        {
